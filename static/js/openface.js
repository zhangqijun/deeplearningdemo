...
$(function () {
    ...
    $(".swiper-container ul li").click(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $(".demo-container").children("ul").append("<li><canvas id=topcanvas width=100px height=100px></canvas></li>");
                var topcanvas = document.getElementById("topcanvas");
                var topcontext = topcanvas.getContext('2d');
                topcontext.drawImage(img, 0, 0, 482, 482, 0, 0, 100, 100);
    ...
    $(document).on('click', '.demo-container ul li canvas', function () {
        ...
        if (canvasidd == "topcanvas") {
            ...
            $(".kpointvalue").text(JSON.stringify(faceapi.faces[id_canvas].kpoint));
            $(".gazexyzvalue").text(JSON.stringify(faceapi.faces[id_canvas].gaze));
            $(".gazeangelvalue").text(JSON.stringify(faceapi.faces[id_canvas].gazeangel));
            $(".posevalue").text(JSON.stringify(faceapi.faces[id_canvas].pose));
            $(".pscalevalue").text(JSON.stringify(faceapi.faces[id_canvas].pscale));
            $(".prvalue").text(JSON.stringify(faceapi.faces[id_canvas].pr));
            $(".ptvalue").text(JSON.stringify(faceapi.faces[id_canvas].pt));
            $(".pvalue").text(JSON.stringify(faceapi.faces[id_canvas].p));
            $(".AUvalue").text(JSON.stringify(faceapi.faces[id_canvas].AU));
            $(".kpoint3dvalue").text(JSON.stringify(faceapi.faces[id_canvas].kpoint3D));
            $(".eyekpointvalue").text(JSON.stringify(faceapi.faces[id_canvas].eye_lmk));
            $(".eyekpoint3dvalue").text(JSON.stringify(faceapi.faces[id_canvas].eye3dlmk));
            /************************画关键点************************/
            $(".face-top").empty();
            ...
            if (zhixing) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        var topcanvas = document.getElementById("topcanvas");
                        var topcontext = topcanvas.getContext('2d');
                        topcontext.drawImage($(".face-top").children("img")[0], 0, 0, 482, 482, 0, 0, 100, 100);
                        $(".faceapi").html(JSON.stringify(data));
                        $(".confidencevalue").html("");
                        $(".bboxvalue").text("");
                        $(".kpointvalue").text("");
                        data.faces.forEach(function (value, index, array) {
                            ...
                            var canvas2 = document.getElementById("canvas" + index);
                            var context2 = canvas2.getContext('2d');
                            context2.drawImage($(".face-top").children("img")[0], bbox.x1, bbox.y1, bbox.x2 - bbox.x1, bbox.y2 - bbox.y1, 0, 0, 100, 100)
                        })
                    }
                });
            }
