...
$(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status) {
                    ...
                    $(".infor_message").empty();
                    var message = "<p>左侧显示为直接放大图片结果，查看超分辨率生成点击上图。</p>";
                    sessionStorage.setItem("faceapi", JSON.stringify(data));
                    $(".infor_message").append(message);
                    $(".face-top").empty();
                    $(".face-top").append("<li><img src=/static/tmpimg/" + data.name + "_srgan_LR.png ></li>");
                    $(".demo-container").children("ul").empty();
                    $(".demo-container").children("ul").append("<li><img src=/static/tmpimg/" + data.name + "_srgan_SR.png ></li>");
    ...
    $(document).on('click', '.demo-container ul li img', function () {
        ...
        console.log(imgsrc);
        $(".face-top li").children()[0].src = imgsrc;
