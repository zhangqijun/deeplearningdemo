...
$(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status) {
                    ...
                    $(".infor_message").empty();
                    var message = "<p>已将图中主要人物进行了3D生成，点击上方图片查看结果信息。您可将以上信息传给其他API进行后续处理和分析。</p>";
                    sessionStorage.setItem("faceapi", JSON.stringify(data));
                    $(".infor_message").append(message);
                    $(".demo-container").children("ul").empty();
                    $(".demo-container").children("ul").append("<li><img src=/static/tmpimg/" + data.name + "/" + data.name + "_prnet_texture.png ></li>");
    ...
    $(document).on('click', '.demo-container ul li img', function () {
        ...
        var data = JSON.parse(sessionStorage.getItem("faceapi"));
        $(".face-top").empty();
        $(".face-top").append("<iframe height='484' width='484' src=static/tmpimg/" + data.name + "/" + data.name + "_prnet.html></iframe>");
        $(".kptvalue").text(JSON.stringify(data.res))
    });
