...
$(function () {
    ...
    $(".swiper-container ul li").click(function () {
        ...
        $.ajax({
            ...
            data: {
                ...
                hairc: $('#hairc')[0].style.backgroundColor,
                uplipc: $('#uplipc')[0].style.backgroundColor,
                downlipc: $('#downlipc')[0].style.backgroundColor
            },
            success: function (data) {
                if (data.status) {
                    ...
                    $(".infor_message").empty();
                    var message = "<p>已检测到图中主要人脸，点击上方图片查看结果信息。您可将以上信息传给其他API进行后续处理和分析。</p>";
                    $(".infor_message").append(message);
                    $(".demo-container").children("ul").empty();
                    $(".demo-container").children("ul").append("<li><img src=/" + data.name + " ></li>");
    ...
    $(document).on('click', '.demo-container ul li img', function () {
        ...
        var imgsrco = $(".face-top").children()[0].src
        var imgsrc = $(this)[0].src;
        $(".face-top").children()[0].src = imgsrc;
        let div_display_jQ_ele = $(".face-top");
        div_display_jQ_ele.on('mouseup', function () {
            div_display_jQ_ele.html('');
            var html = '<img src="' + imgsrc + '" alt="" id="img_display">';
            div_display_jQ_ele.html(html);
        });
        div_display_jQ_ele.on('mousedown', function () {
            div_display_jQ_ele.html('');
            var html = '<img src="' + imgsrco + '" alt="" id="img_display">';
            div_display_jQ_ele.html(html);
        });
    });
    ...
    $("#file").change(function (e) {
        ...
        $(".face-top").children("img")[0].onload = function () {
            if (zhixing) {
                ...
                var hairc = $('#hairc')[0].style.backgroundColor;
                var uplipc = $('#uplipc')[0].style.backgroundColor;
                var downlipc = $('#downlipc')[0].style.backgroundColor;
                var formData = new FormData();
                formData.append("base64", base64img);
                formData.append("hairc", hairc);
                formData.append("uplipc", uplipc);
                formData.append("downlipc", downlipc);
                $.ajax({
                    url: globalurl,
                    method: "POST",
                    cache: false,
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.status) {
                            ...
                            $(".infor_message").css("display", "block");
                            $(".infor_message").empty();
                            var message = "<p>已将图中主要人脸进行化妆，点击上方图片查看结果信息。</p>";
                            $(".infor_message").append(message);
                            $(".demo-container").children("ul").empty();
                            $(".demo-container").children("ul").append("<li><img src=/" + data.name + " ></li>");
                            $(".faceapi").html(JSON.stringify(data));
